...
from mcdreforged.api.all import RColor, RTextBase, RTextTranslation, RText
...
class LegacyDimension(Dimension):
    ...
    def get_opposite(self, pos: Position) -> Tuple['Dimension', Position]:
        ...
        raise RuntimeError(
            'Legacy dimension -1 (the end) does not have opposite dimension')
...
class CustomDimension(Dimension):
    ...
    def get_id(self) -> str:
        raise RuntimeError(
            'Custom dimension {} does not have integer id'.format(self.reg_key))
    ...
    def get_opposite(self, pos: Position) -> Tuple['Dimension', Position]:
        raise RuntimeError(
            'Custom dimension {} does not have opposite dimension'.format(self.reg_key))
